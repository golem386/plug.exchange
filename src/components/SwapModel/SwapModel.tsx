...
const Swapmodel = (props: ComponentProps) => {
  ...
  const onModel = (article: any) => { dispatch(onModelOpen(article)) }
  const onSelectIcon = (coin: any) => { dispatch(onSelectCoin(coin)) }
  const onReceiveIcon = (coin: any) => { dispatch(onReceiveCoin(coin)) }
  const CoinDetail: any = useSelector(
    (state: ArticleState) => state.CoinDetail
  )
  const ReceiveCoin: any = useSelector(
    (state: ArticleState) => state.receiveCoinDetail
  )
  ...
  return (
    <>
      <MainDiv>
        <SwapDiv>
          <Swap>Swap</Swap>
          <img src={menu} onClick={() => { onModel("Transaction") }} />
        </SwapDiv>
        <SwapDiv>
          <Pay>You Pay</Pay>
          <Max>Max 0 ETH</Max>
        ...
        <MainViewInputToken>
          ...
          <FormControl sx={{ m: 1, minWidth: 120 }}>
            <SelectMain onClick={() => { onModel("SelectToken") }}>
        ...
        <ReceiveMain>
          <UpDownImage src={UpDown} onClick={() =>{onChangeToken()}} />
          <YouReceive>You Receive</YouReceive>
          <MainViewInput>
            <TextInput placeholder="0" />
            <FormControl sx={{ m: 1, minWidth: 120 }}>
              <SelectMainDarkCoin onClick={() => { onModel("ReceiveToken") }}>
        ...
        {
          props.btnTitle === 'Connect Wallet' ? <><MainContent>
            <ShortView>
              <ETH>1 ETH = 4,008.2766 USDT</ETH>
              <ImgVarticalShort src={ShortVartical} />
            </ShortView>
            <ImgDownAero onClick={() => { setSlider(!Slider) }} src={Slider === true ? TopAero : DownAero} />
          </MainContent>
            {
              Slider === true ? <><UsdtList>
                <MainTitleView>
                  <Div>
                    <Title>1inch</Title>
                ...
                <AutoRoute>
                  ...
                  {
                    AddEntey === true ? <IconHr onClick={() => { setAddEntey(!AddEntey) }} src={hr} /> : <Icon onClick={() => { setAddEntey(!AddEntey) }} src={plus} />
                  }
                </AutoRoute>
                {
                  AddEntey === true ? <Route>
        ...
        <OrderBtn>{props.btnTitle}</OrderBtn>
      </MainDiv>
      {
        props.btnTitle === 'Connect Wallet' ? <DetailView>
          <Expected>
